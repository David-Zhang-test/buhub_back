...
type ImportMode = "append" | "replace_by_category";
...
type RatingRow = {
  ...
  semester?: string;
  score_1_key?: string;
  score_1_value?: number | string;
  score_2_key?: string;
  score_2_value?: number | string;
  score_3_key?: string;
  score_3_value?: number | string;
  tags?: string;
  comment?: string;
  created_at?: string;
  updated_at?: string;
...
function parseArgs(): ParsedArgs {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--mode" && args[i + 1]) {
      const next = args[i + 1] as ImportMode;
...
function parseCategory(value: unknown): RatingCategory {
...
async function syncSeedUserEmail(userId: string, rawEmail: unknown) {
...
function toScoreValue(value: unknown): number {
...
function loadSheet<T>(workbook: XLSX.WorkBook, sheetName: string): T[] {
  const sheet = workbook.Sheets[sheetName];
  if (!sheet) return [];
  return XLSX.utils.sheet_to_json<T>(sheet, { defval: "" });
}

async function resolveUsers(userRows: UserRow[]) {
...
async function importRatingItems(rows: RatingItemRow[]) {
...
async function importScoreDimensions(rows: ScoreDimensionRow[]) {
  for (const row of rows) {
    const category = parseCategory(row.category);
    const name = sanitizeText(row.name);
    if (!name) {
...
function buildScores(row: RatingRow): Record<string, number> {
...
async function validateRatingDimensions(category: RatingCategory, scores: Record<string, number>) {
...
async function replaceImportedRatingsByCategories(categories: RatingCategory[]) {
...
async function importRatings(rows: RatingRow[], userMap: Map<string, string>, mode: ImportMode) {
  const seenCompositeKeys = new Set<string>();
  const categoriesForReplace: RatingCategory[] = [];
...
function summarize(rows: {
  ...
  scoreDimensions: ScoreDimensionRow[];
  users: UserRow[];
  ratings: RatingRow[];
}) {
...
async function main() {
  ...
  const ratingItems = loadSheet<RatingItemRow>(workbook, "RatingItems");
  const scoreDimensions = loadSheet<ScoreDimensionRow>(workbook, "ScoreDimensions");
  const users = loadSheet<UserRow>(workbook, "Users");
  const ratings = loadSheet<RatingRow>(workbook, "Ratings");
