...
export const COURSE_CODE_PATTERN = /^[A-Z]{2,4}[-\s]*\d{3,4}$/;

// ─── Room code pattern (D-04) ──────────────────────────────────────────────
// Matches 2-3 uppercase letters + 3-4 digits (no separator).
export const ROOM_CODE_PATTERN = /^[A-Z]{2,3}\d{3,4}$/;
...
function isLikelyRoomCode(text: string): boolean {
  ...
  return KNOWN_ROOM_PREFIXES.includes(prefix);
}
...
export function identifyCourses(cards: CardInput[]): ParsedCourse[] {
  ...
  for (const card of cards) {
    ...
    const processedTexts = mergeAdjacentCourseTokens(stripped);
